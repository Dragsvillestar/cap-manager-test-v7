...
const apiUrl = "https://employee-tracker-test.onrender.com";
...
async function initMapAndFetchLocation() {
    // ✅ Access Geolocation from Capacitor Plugins
...
async function updateClockButton() {
    ...
    try {
        const response = await fetch(`${apiUrl}/get-last-clock-event?userId=${userId}`);
...
async function fetchClockEvents(userId,username) {
    try {
        const response = await fetch(`${apiUrl}/get-clock-events`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
                userId: userId 
...
async function fetchLastClocking(userId) {
    try {
        const response = await fetch(`${apiUrl}/get-last-clocking`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ userId }),
        });
...
async function fetchUserDataWithSubordinates(userId) {
    try {
        const response = await fetch(`${apiUrl}/getUserDataWithSubordinates`, {
            method: "POST",
            headers: {
